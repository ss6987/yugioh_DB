...
def getRarity(rarity_string):
    if rarity_string in ["20thシク", "20thシークレット", "20th シークレット", "20th"]:
    ...
    elif rarity_string in ["レリーフ", "レリーフレア", "アルティメトレア", "アルティメット(レリーフ)", "アルティメット", "レリ", "Ultimate", "RR", "UTR"]:
    ...
    elif rarity_string in ["Collectors", "CR"]:
        return raritys.filter(rarity="コレクターズレア").first()
    elif rarity_string in ["ゴールドシークレット", "GSC"]:
    ...
    elif rarity_string in ["ウルトラシークレットレア", "Uシク", "シク", "シークレット", "Secret", "SC"]:
        return raritys.filter(rarity="シークレットレア").first()
    elif rarity_string in ["SR", "スーパー", "Super"]:
        return raritys.filter(rarity="スーパーレア").first()
    elif rarity_string in ["ノーマルレア", "N", "NR", "ノーレア", "ノーマルカード", "N-Rare","ノーマルトレア"]:
        return raritys.filter(rarity="ノーマル").first()
    elif rarity_string in ["ノーマルパラレル", "Nパラ", "パ", "パラ", "ノーマルパラレルレア", "ノーパラ", "NP", "PR", "NPR"]:
        return raritys.filter(rarity="パラレル").first()
    elif rarity_string in ["ウルトラレアパラレル", "URパラ", "ウルトラパラレル", "ウルトラパラレルレア", "UPR"]:
    ...
    elif rarity_string in ["ホロ", "ホログラフック", "ホログラフックレア", "ホログラフィック", "Holographic","HR"]:
        return raritys.filter(rarity="ホログラフィックレア").first()
    elif rarity_string in ["ミレニアムレア", "スペシャルパラレル", "Millennium", "MR", "ミレニアム仕様"]:
